...
var App = (function () {
    function App() {
        ...
        /* Globals           */
        /*********************/
        this._calloutEnabled = false; // we only want to add callout when an image is loaded.
        ...
        Office.initialize = function (reason) {
            $(document).ready(function () {
                ...
                if (Office.context.requirements.isSetSupported("WordApi", 1.2)) {
                    ...
                    $('#loadSelectedImage').click(_this.loadSelectedImageHandler.bind(_this));
                    $('#insertImageAtSelection').click(_this.insertImageHandler.bind(_this));
                    // Scale the size of the canvas so that it scales  
                    // when a user resizes the add-in.
                    window.addEventListener('resize', _this.resizeCanvas.bind(_this), false);
    ...
    App.prototype.initCanvas = function () {
        ...
        ctx.canvas.addEventListener('click', function (event) {
            ...
            if (_this._calloutEnabled) {
                ...
                var height = _this._windowWidth * _this._resizeRatio;
                var mouseX = (event.clientX - canvasBounds.left) * canvas.width / _this._windowWidth;
                ...
                var width = ctx.measureText(_this._calloutNumber.toString());
                ctx.font = 'bold 16px calabri ';
                ctx.fillStyle = 'white';
                ctx.textAlign = 'center';
                ctx.fillText(_this._calloutNumber.toString(), mouseX, mouseY + (radius / 3)); // this last argument is approximately correct for placement.
            }
        });
    };
    /**
    ...
    */
    ...
    App.prototype.loadImageIntoCanvas = function (base64EncodedImage) {
        ...
        this._image.onload = function () {
            var canvas = document.getElementById('canvas');
            var ctx = canvas.getContext("2d");
            canvas.height = _this._image.height;
            canvas.width = _this._image.width;
    ...
    */
    App.prototype.loadSelectedImageHandler = function () {
        var _this = this;
            ...
            return context.sync()
                ...
                if (imageRange.inlinePictures.items.length === 1) {
                    ...
                    return context.sync().then(function () {
                        _this.loadImageIntoCanvas(imageString);
                    });
                }
                else {
    ...
    */
    App.prototype.insertImageHandler = function () {
        var _this = this;
        ...
        if (this._image) {
            ...
            Word.run(function (context) {
                ...
                return context.sync()
                    ...
                    if (_this._calloutNumber > 0) {
                        var lastParagraph = insertedImage.insertParagraph('Here are your callout descriptions:', Word.InsertLocation.after);
                        for (var i = 0; i < _this._calloutNumber; i++) {
